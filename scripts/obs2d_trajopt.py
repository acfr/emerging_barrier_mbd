...
def optimize_and_rollout(
    trajopt, state_init, start_seed, num, emerging_barrier=True, **kwargs
):
...
def run_trajectory_comparison(state_init, start_seed, num, sys, sys_lc, opt, horizon):
    ...
    experiments = [
        ...
        },
        {
            ...
            "kwargs": {"emerging_barrier": False, "normalising_factor": 5},
        },
        {
    ...
    for exp in experiments:
        trajs = optimize_and_rollout(
            exp["trajopt"], state_init, start_seed, num, **exp["kwargs"]
        )
        graph_trajectory_distribution(trajs, exp["name"], exp["sys"])


def collect_data_for_alpha(
    alpha,
    num_seeds,
    sample_size,
    temperature,
    ...
    for seed in range(num_seeds):
        ...
        constraint_violations = [s.constraint_violations for s in opt.state_history]
        mean_costs = [s.mean_cost for s in opt.state_history if s.mean_cost < 10000]

        all_constraint_violations.append(constraint_violations)
        all_mean_costs.append(mean_costs)

    # Average across seeds
    avg_constraint_violations = jnp.mean(jnp.array(all_constraint_violations), axis=0)

    # Average mean costs across seeds (handle different lengths)
    max_len = max(len(costs) for costs in all_mean_costs)
    padded_costs = [
        costs + [costs[-1]] * (max_len - len(costs)) if len(costs) < max_len else costs
...
def run_statistics_experiment(state_init, sys, kwargs, normalising_factor):
    ...
    for config in plot_configs:
        ...
        for i, alpha in enumerate(alphas):
            data = (
                all_avg_constraint_violations[i]
                if config["data_key"] == "constraint_violations"
                else all_avg_mean_costs[i]
            )
            normalize_factor = (
                inv_sample_size_percent if config.get("normalize", False) else 1
            )
            plt.plot(
                t[: len(data)], data / normalize_factor, label=f"$\\kappa={alpha}$"
            )
        ...
        plt.savefig(f"{save_path_base}{config['name']}", dpi=300, bbox_inches="tight")
        plt.close()
    ...
    plt.scatter(
        corresponding_alphas,
        flatten(final_mean_costs),
        s=100,
        alpha=0.7,
    ...
    plt.grid(True, alpha=0.3)
    plt.savefig(
        f"{save_path_base}final_min_costs_vs_alpha.pdf", dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.savefig(f"{save_path_base}constraint_violations_progress_unconstrained.pdf")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("command", choices=["statistics", "comparison", "time"])
    ...
    opt = DiffusionOptimiser(temperature=0.1, optimisation_steps=200, sample_size=440)
